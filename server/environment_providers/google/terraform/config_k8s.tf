...
resource "google_compute_instance" "google_nodes" {
  ...
  attached_disk {
    source      = google_compute_disk.longhorn[each.key].name
    device_name = google_compute_disk.longhorn[each.key].name
  }
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "sudo chown -R ${var.google_user}:${var.google_user} /var/lib/longhorn",
      "sudo chmod 777 /var/lib/longhorn",
      "sudo mkfs.ext4 -m 0 -E lazy_itable_init=0,lazy_journal_init=0,discard /dev/disk/by-id/google-${google_compute_disk.longhorn[each.key].name}",
      "sudo echo '/dev/disk/by-id/google-${google_compute_disk.longhorn[each.key].name} /var/lib/longhorn ext4 defaults 0 0' | sudo tee -a /etc/fstab",
      "sudo mount -a",
    ]
  }
...
resource "google_compute_disk" "longhorn" {
